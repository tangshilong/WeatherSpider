...
public class POIUtil {
    ...
    public static void main(String[] args) {
        File file = new File("C:\\Users\\tangshilong\\Desktop\\改正过中国地面气候资料国际交换站数据集台站信息.xlsx");
        try {
            List<String[]> list = POIUtil.readExcel(file);
            for (String[] row : list) {

                Station station = new Station(row[1] + "", (row[2] + "").replace(" ", ""), row[3] + "", row[9] + "", row[10] + "", row[6] + "");
                //byte[] b1 = row[2].getBytes("UTF-8");
